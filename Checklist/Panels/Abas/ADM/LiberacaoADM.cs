...
{
    public partial class LiberacaoADM : Form
    {
        public LiberacaoADM()
        {
            InitializeComponent();
        ...
        int DistanceUnity = 1;
        private void LiberacaoADM_Load(object sender, EventArgs e)
        {
            ...
            if(cliente != null)
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT Checklist.*, Itens.* FROM Checklist INNER JOIN Itens ON Checklist.id_item = Itens.id_item WHERE Checklist.id_cliente = @id_cliente AND Itens.tipo LIKE '%Liberação%'", conn);
