...
namespace Checklist.Panels.Abas.User
{
    public partial class AbasUser : Form
    {
        public AbasUser()
        {
            InitializeComponent();
        ...
        int DistanceUnity = 1;
        private void Abas_Load(object sender, EventArgs e)
        {
            ...
            if (cliente != null)
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT Checklist.*, Itens.* FROM Checklist INNER JOIN Itens ON Checklist.id_item = Itens.id_item WHERE Checklist.id_cliente = @id_cliente AND Itens.tipo LIKE @aba", conn);

                cmd.Parameters.AddWithValue("@id_cliente", cliente.getId());
                cmd.Parameters.AddWithValue("@aba", "%" + aba.getNome() + "%");
                ...
                if (dt.Rows.Count == 0) // Se não houver linhas retornadas pela consulta
                {
                    ...
                    lblEmpty.Location = new Point(95, 100); // Posição do Label
                    return;
                }
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    button.Top = currentTop; // Posiciona o botão na mesma posição Y do rótulo
                    button.Left = lbl.Right - 115; // Posicionamento ao lado direito do Label, com um pequeno espaço
                    button.Height = 25;
                    button.Width = 85; // Largura do botão
                    button.Text = "Detalhes"; // Texto do botão
                    button.Font = new Font("Arial", 10.25F, FontStyle.Bold, GraphicsUnit.Point, ((Byte)(0)));
                    button.Tag = itemId;
                    button.Click += BtnDetalhes_Click; // Evento de clique do botão
                    button.BringToFront();
        ...
        private void CheckBox_CheckedChanged(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection("Data Source=.;initial catalog=Checklists;integrated security=true;"))
            {
                ...
                string sql = "UPDATE Checklist SET condicao=@condicao WHERE id_item=@id_item";
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            DetalheChecklist detalheChecklist = new DetalheChecklist();
                            detalheChecklist.Show();
                        }
                    }
                }
            }
